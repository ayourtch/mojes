...
use syn::token::Comma;
use syn::{Block, Expr, Fields, ItemEnum, ItemFn, ItemStruct, Pat, Stmt, Type, parse_macro_input};
...
fn handle_format_macro(args: &Punctuated<Expr, Comma>) -> String {
    if args.is_empty() {
        return "\"\"".to_string();
    }
...
fn rust_block_to_js(block: &Block) -> String {
...
fn rust_expr_to_js(expr: &Expr) -> String {
...
fn get_js_type(rust_type: &str) -> &'static str {
    match rust_type {
...
fn generate_js_class_for_struct(input_struct: &ItemStruct) -> String {
    let struct_name = input_struct.ident.to_string();
...
fn generate_js_enum(input_enum: &ItemEnum) -> String {
    let enum_name = input_enum.ident.to_string();
...
fn format_rust_type(ty: &Type) -> String {
    match ty {
        Type::Path(type_path) => {
